...
class CourseController extends Controller
{
    ...
    {
        $courseindexinsert = new Courses;
        $url = url('/Sub-admin/create-course');
        $title = "Add Course + ";
        $data = compact('url', 'title', 'courseindexinsert');
        return view('Sub-admin/create-course')->with($data);
    }
    public function editSbCourse($course_id)
    {
        $courseindexinsert = Courses::find($course_id);

        if (is_null($courseindexinsert)) {
            return redirect('/Sub-admin/viewCourse');
        } else {
            $url = url('/Sub-admin/update-course') . "/" . $course_id;
            $title = "Edit Course Details ";
            $data = compact('url', 'title', 'courseindexinsert');
            return view('/Sub-admin/create-course')->with($data);
        }
    }
    public function updateSbCourse($course_id, Request $request)
    {
        $courseindexinsert = Courses::find($course_id);
        $courseindexinsert->course_name = $request->input('course_name');
        $courseindexinsert->course_desc = $request->input('course_desc');
    ...
    public function deleteSbCourse($course_id)
    {
        Courses::find($course_id)->delete();
        return redirect()->back();
    }
    ...
    public function displaySbCourse()
    {
        $courseindexinserts = Courses::all();
        $courseindexinsertss = compact('courseindexinserts');
        return view('Sub-admin.viewCourse')->with($courseindexinsertss);
    }

    
    public function displaycontactexperts()
    {
        $courseindexinserts = Courses::all();
        $courseindexinsertss = compact('courseindexinserts');
        return view('contact-experts')->with($courseindexinsertss);
    }
}
